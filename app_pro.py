...
請針對這份生物講義的【第 {target_page} 頁】進行教學導讀。所有的內容必須超過500個字，內容不可造假或有幻覺亂講。
...
課前熱身：開場白一定要很生活化，提到「現炸大雞排」配「波霸奶茶」，並跟同學打招呼（可以提到最近備課講太多話，聲音有點沙啞，拉近距離）。劇本開頭必喊：『各位同學，請翻開講義第 {target_page} 頁。』
重點整理詳細解析：用自然段落解釋畫面上的圖表與核心觀念。生物科圖表非常多，請「務必」把圖表代表的生命現象與意義說清楚。拒絕唸出圖片的純排版描述。
題目講解：若頁面中有練習題，請詳細講解每個選項為什麼對或錯（分段配速解說）。若該頁無題目，則帶領學生做該頁的重點觀念總結。
常考重點與易錯提醒：點出會考/大考最愛考的重點，以及學長姐最常搞混的地方（例如：動植物細胞比對、光合作用變因、消化液作用等避坑指南）。結尾必含句：「喝口珍奶，我們準備進入下一個生命真理！」
"""
